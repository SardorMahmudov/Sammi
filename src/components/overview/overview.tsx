...
const Overview = () => {
  ...
  return (
    <>
      <Text mt={3}>
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Omnis tempora itaque expedita rem eveniet fugit! Mollitia
        expedita corporis eligendi earum, beatae cumque ipsum, alias neque ab facilis natus nesciunt commodi!
      </Text>
      ...
      <Box mt={3}>
        {requirement.split(", ").map((text, idx) => (
          <Flex key={idx} gap={2} align={"center"}>
            <Icon as={GoProjectSymlink} w={5} h={5} />
            <Text>{text}</Text>
          </Flex>
        ))}
      </Box>
